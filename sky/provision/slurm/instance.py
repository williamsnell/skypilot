...
def _create_virtual_instance(
    ...
    if (accelerator_type is not None and accelerator_type.upper() != 'NONE' and
        ...
        if accelerator_type == 'GPU':
            # 'GPU' is the synthetic name for untyped GRES (e.g.
            # 'gpu:4'). Use gpu:<count> without a type qualifier.
...
"""
        ...
        if container_runtime == 'podman-hpc':
            ...
            container_block = (
                ...
                f'# Query allocated memory from Slurm so podman-hpc can\n'
                f'# enforce it. Prevents the container from exceeding the\n'
                f'# job allocation (e.g. 115GB per GPU on Isambard).\n'
                f'# Extract from AllocTRES (not ReqTRES) for actual.\n'
                f'SLURM_ALLOC_MEM=$(scontrol show job $SLURM_JOBID -o | '
                f'grep -o "AllocTRES=[^ ]*" | '
                f'grep -o "mem=[0-9]*" | '
                f'grep -o "[0-9]*")\n'
                f'PODMAN_MEM_ARG=""\n'
                f'if [ -n "$SLURM_ALLOC_MEM" ]; then\n'
                f'  PODMAN_MEM_ARG="--memory ${{SLURM_ALLOC_MEM}}m"\n'
                f'fi\n'
                f'echo "[container] Initializing on all nodes"\n'
                f'srun --overlap '
                f'{"--label " if num_nodes > 1 else ""}--unbuffered '
                f'--nodes={num_nodes} --ntasks-per-node=1 '
                f'podman-hpc run --gpu $PODMAN_MEM_ARG '
