...
class SlurmInstanceType:
    ...
    When CPU and memory are None (GPU-only), the name is just "{type}:{count}"
    (e.g. "GPU:1"). This lets the Slurm scheduler auto-allocate CPU/memory
    per GPU (e.g. per-GPU superchip allocation on Isambard).

    Examples:
        - 4CPU--16GB
        - 0.5CPU--1.5GB
        - 4CPU--16GB--V100:1
        - GPU:1           (GPU-only, no explicit CPU/memory)
        - H100:4          (GPU-only, typed)
    """
    ...
    def name(self) -> str:
        ...
        if self.cpus is not None and self.memory is not None:
            ...
            return name
        # GPU-only: no CPU/memory prefix.
        assert self.accelerator_count is not None, (
            'Instance type must have either CPU/memory or accelerators')
        assert self.accelerator_type is not None, self.accelerator_count
        acc_name = self.accelerator_type.replace(' ', '_')
        return f'{acc_name}:{self.accelerator_count}'

    # Matches GPU-only format: "TYPE:COUNT" (e.g. "GPU:1", "H100:4")
    _GPU_ONLY_PATTERN = re.compile(
        r'^(?P<accelerator_type>[\w\d-]+):(?P<accelerator_count>\d+)$')
    # Matches CPU/memory format with optional accelerator suffix.
    _FULL_PATTERN = re.compile(
        r'^(?P<cpus>\d+(\.\d+)?)CPU--(?P<memory>\d+(\.\d+)?)GB'
        r'(?:--(?P<accelerator_type>[\w\d-]+)'
        r':(?P<accelerator_count>\d+))?$')

    @classmethod
    def is_valid_instance_type(cls, name: str) -> bool:
        """Returns whether the given name is a valid instance type."""
        return (bool(cls._FULL_PATTERN.match(name)) or
                bool(cls._GPU_ONLY_PATTERN.match(name)))
    ...
    def _parse_instance_type(
        ...
        Returns:
            cpus: Number of CPUs (None for GPU-only types)
            memory: Amount of memory in GB (None for GPU-only types)
            accelerator_count: Number of accelerators
            accelerator_type: Type of accelerator
        """
        # Try full format first (has CPU/memory).
        match = cls._FULL_PATTERN.match(name)
        ...
        # Try GPU-only format (no CPU/memory).
        match = cls._GPU_ONLY_PATTERN.match(name)
        if match is not None:
            accelerator_type = match.group('accelerator_type').replace(' ', '_')
            accelerator_count = int(match.group('accelerator_count'))
            return None, None, accelerator_count, accelerator_type
    ...
    def from_resources(cls,
        ...
        acc_count: Optional[int] = math.ceil(accelerator_count)
        acc_type: Optional[str] = accelerator_type or None
        if acc_count == 0:
            acc_count = None
            acc_type = None
        return cls(cpus=cpus,
...
def _check_cpu_mem_fits(
    ...
    for node_info in node_list:
        ...
        # GPU-only instance types have cpus=None, memory=None — skip
        # the CPU/memory fit check and let the scheduler handle it.
        req_cpus = candidate_instance_type.cpus
        req_mem = candidate_instance_type.memory
        if ((req_cpus is None or node_cpus >= req_cpus) and
            (req_mem is None or node_mem_gb >= req_mem)):
            return True, None
